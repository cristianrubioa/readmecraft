...
import { styles, TEMPLATES_ORDER, PROJECTS_PER_PAGE, MAX_SELECTED_REPOS } from './utils/constants'
import type { TemplateName, SectionState, GithubRepo } from './types/github'
import { standardTemplate } from './templates/standard'

function App() {
  const [isDark, setIsDark] = useDarkMode()
  const [bannerTheme, setBannerTheme] = useState<'dark' | 'light'>(() => isDark ? 'dark' : 'light')
  ...
  const [sections, setSections] = useState<SectionState>(standardTemplate.sections)
  const [selectedRepos, setSelectedRepos] = useState<number[]>([])
  const [projectsPage, setProjectsPage] = useState(0)

  const { user, loading: userLoading, error: userError } = useGithubUser(searchUsername)
  const { repos, loading: reposLoading, error: reposError } = useGithubRepos(searchUsername)
  ...
  const handleRepoToggle = (repoId: number) => {
    setSelectedRepos((prev) =>
      prev.includes(repoId)
        ...
        : prev.length < MAX_SELECTED_REPOS ? [...prev, repoId] : prev
    )
  }
  ...
  useEffect(() => {
    ...
    const projectRepos: GithubRepo[] = selectedRepos.length > 0
      ? selectedRepos.map((id) => repos.find((r) => r.id === id)).filter((r): r is GithubRepo => r !== undefined)
      : [...repos].sort((a, b) => b.stargazers_count - a.stargazers_count).slice(0, 3)
    const svg = generateBanner(user, repos, sections, contribData, projectRepos, bannerTheme)
  ...
  const markdown =
    user && repos.length ? generateMarkdown(user, repos, sections, selectedRepos, './banner.svg') : ''
        ...
        <div className="max-w-6xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <i className={isDark ? 'fas fa-sun text-lg' : 'fas fa-moon text-lg'} />
              </button>
              {user && (
                <button
                  onClick={handleReset}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-stretch">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="backdrop-blur-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl p-8 text-center shadow-lg shadow-slate-200/50 dark:shadow-slate-900/50">
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-300 mb-4">@{user.login}</p>
                  {user.bio && (
                ...
                <div className={styles.card + ' p-6'}>
                  ...
                  <SectionEditor sections={sections} onSectionChange={handleSectionChange} />
                </div>
                  ...
                  <div className={styles.card + ' p-6'}>
                    ...
                    <div className="space-y-2">
                      {repos
                        ...
                        .map((repo) => {
                          ...
                          const isDisabled = !isChecked && selectedRepos.length >= MAX_SELECTED_REPOS
                          return (
                            <label
                              key={repo.id}
                    ...
                    {Math.ceil(repos.length / PROJECTS_PER_PAGE) > 1 && (
                      <div className="flex gap-2 mt-4 justify-center">
                        ...
                        <button
                          onClick={() => setProjectsPage(Math.min(Math.ceil(repos.length / PROJECTS_PER_PAGE) - 1, projectsPage + 1))}
              ...
              <div className="lg:col-span-3 flex flex-col gap-6 h-full">
                ...
                <div className="flex-1 min-h-0 backdrop-blur-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl overflow-hidden flex flex-col shadow-lg shadow-slate-200/50 dark:shadow-slate-900/50">
                  <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-700 shrink-0">
                    <h3 className="text-lg font-light text-slate-900 dark:text-slate-100">Preview</h3>
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => setBannerTheme((t) => (t === 'dark' ? 'light' : 'dark'))}
                        ...
                        title={bannerTheme === 'dark' ? 'Switch to light banner' : 'Switch to dark banner'}
                      >
                        <i className={bannerTheme === 'dark' ? 'fas fa-moon' : 'fas fa-sun'} />
                      </button>
                      {bannerBlobUrl && (
                        <button
                  ...
                  <div className="flex-1 overflow-auto bg-slate-50 dark:bg-slate-900 p-4">
                    {bannerBlobUrl && (
                      <img src={bannerBlobUrl} width="100%" style={{ borderRadius: '8px', display: 'block' }} />
                    )}
                  </div>
                </div>
                ...
                <div className="shrink-0 backdrop-blur-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl overflow-hidden shadow-lg shadow-slate-200/50 dark:shadow-slate-900/50">
                  <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-700">
                    <h3 className="text-sm font-light text-slate-900 dark:text-slate-100">README.md</h3>
                ...
                <div className="shrink-0 backdrop-blur-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl p-6 shadow-lg shadow-slate-200/50 dark:shadow-slate-900/50">
                  <h3 className="text-lg font-light text-slate-900 dark:text-slate-100 mb-4">How to use</h3>
                  <p className="text-sm font-light text-slate-400 dark:text-slate-500 mb-2">Manual</p>
                  <ol className="space-y-2 text-sm font-light text-slate-600 dark:text-slate-400">
                    <li className="flex gap-3">
                      <span className="text-slate-400 dark:text-slate-600 shrink-0">1.</span>
                      <span>Click <strong className="font-medium text-slate-700 dark:text-slate-300">Download</strong> to save <code className="text-xs bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded">banner.svg</code></span>
                    </li>
                    <li className="flex gap-3">
                      <span className="text-slate-400 dark:text-slate-600 shrink-0">2.</span>
                      <span>Click <strong className="font-medium text-slate-700 dark:text-slate-300">Copy</strong> to copy the <code className="text-xs bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded">README.md</code> snippet</span>
                    </li>
                    <li className="flex gap-3">
                      <span className="text-slate-400 dark:text-slate-600 shrink-0">3.</span>
                      <span>Commit both to your <code className="text-xs bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded">{user.login}/{user.login}</code> repo</span>
                    </li>
                  </ol>
                  <div className="mt-4 space-y-0">
                    <div className="pt-4 border-t border-slate-100 dark:border-slate-700">
                      <p className="text-sm font-light text-slate-400 dark:text-slate-500 mb-2">Auto-update with CLI</p>
                      <ol className="space-y-2 text-sm font-light text-slate-600 dark:text-slate-400">
                        <li className="flex gap-3"><span className="text-slate-400 dark:text-slate-600 shrink-0">1.</span><span>Run <code className="text-xs bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded">make setup</code> and set <code className="text-xs bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded">GITHUB_USERNAME</code> and <code className="text-xs bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded">PROFILE_REPO_TOKEN</code> in <code className="text-xs bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded">.env</code></span></li>
                        <li className="flex gap-3"><span className="text-slate-400 dark:text-slate-600 shrink-0">2.</span><span>Edit <code className="text-xs bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded">readmecraft.yml</code> to configure template, theme and projects</span></li>
                        <li className="flex gap-3"><span className="text-slate-400 dark:text-slate-600 shrink-0">3.</span><span><code className="text-xs bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded">make build</code> — preview in <code className="text-xs bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded">output/</code></span></li>
                        <li className="flex gap-3"><span className="text-slate-400 dark:text-slate-600 shrink-0">4.</span><span><code className="text-xs bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded">make generate</code> — push to your profile repo</span></li>
                      </ol>
                    </div>
                    <div className="pt-4 border-t border-slate-100 dark:border-slate-700 mt-4">
                      <p className="text-sm font-light text-slate-400 dark:text-slate-500 mb-2">Auto-update with GitHub Actions</p>
                      <p className="text-sm font-light text-slate-600 dark:text-slate-400">Fork the repo and add <code className="text-xs bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded">PROFILE_REPO_TOKEN</code> as a repository secret — runs every 15 days.</p>
                    </div>
                  </div>
                </div>
