import type { GithubUser, GithubRepo, SectionState, ContribData } from '../types/github'
...
export function generateBanner(
  user: GithubUser,
  repos: GithubRepo[],
  sections: SectionState = { about: true, stats: true, skills: true, projects: true },
  ...
  // Username
  rows.push(`<text x="${PAD}" y="${cy}" font-family="Consolas,'Courier New',monospace" font-size="28" font-weight="700" fill="${t.accent}">${xe(user.login)}</text>`)
  cy += 30
  ...
  if (sections.about) {
    ...
    if (meta.length) {
      rows.push(`<text x="${PAD}" y="${cy}" font-family="Consolas,monospace" font-size="13" fill="${t.subtext}">${meta.join('   ')}</text>`)
      cy += 26
    }
    const bio = user.bio ? trunc(user.bio, 88) : ''
    if (bio) {
      rows.push(`<text x="${PAD}" y="${cy}" font-family="Consolas,monospace" font-size="13" fill="${t.italic}" font-style="italic">"${xe(bio)}"</text>`)
      cy += 26
    }
  }
  ...
  if (langs.length > 0) {
    ...
    rows.push(`<line x1="${PAD}" y1="${divY}" x2="${W - PAD}" y2="${divY}" stroke="${t.divider}" stroke-width="1"/>`)
    rows.push(`<text x="${PAD}" y="${labelY}" font-family="Consolas,monospace" font-size="10" fill="${t.label}" letter-spacing="1.5">TOP LANGUAGES</text>`)
    rows.push(dotsSvg)
    cy += 16
  }
  ...
  if (hasChart) {
    cy += 12
    rows.push(`<line x1="${PAD}" y1="${cy}" x2="${W - PAD}" y2="${cy}" stroke="${t.divider}" stroke-width="1"/>`)
    cy += 16
    ...
    rows.push(`<path d="${line}" fill="none" stroke="${t.accent}" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/>`)
    ...
    for (let i = 0; i < weeks.length; i++) {
      ...
      if (month !== lastMonth) {
        ...
        rows.push(`<text x="${x.toFixed(1)}" y="${labelY}" font-family="Consolas,monospace" font-size="10" fill="${t.label}">${MONTHS[month]}</text>`)
        lastMonth = month
      }
    }
  ...
  if (sections.projects && selectedRepos.length > 0) {
    ...
    rows.push(`<text x="${PAD}" y="${cy}" font-family="Consolas,'Courier New',monospace" font-size="13" font-weight="700" fill="${t.accent}">\u{1F680} FEATURED PROJECTS</text>`)
    cy += 24
